 ...
 * @author wongtiksoon
 */
@WebServlet(name = "RetrieveCustomerName", urlPatterns = {"/RetrieveCustomerName"})
public class RetrieveCustomerName extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (!found.getStatus().equals("Available")){
                String message = "Only available cars can be booked!";
                request.setAttribute("message", message);
                request.getRequestDispatcher("SalesmanSearchCar").include(request, response);
            } else {
                request.setAttribute("cardata", found);
                List<Customer> customerfound = customerFacade.validCustomer();
                request.setAttribute("validcustomer", customerfound);
    ...
    private void alert(String i_am_an_alert_box) {
